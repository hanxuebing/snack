...
const removeNoise = (type) => {
  let inputNoise;
  const e = inputRef.value
  const offsetParent = e.offsetParent
  const lastElementChild = e.lastElementChild
  if (type == "input") {
...
<div class="editor-field editor-field__textbox">
  ...
  <div class="editor-field__container">
    <input type="text" class="editor-field__input" ref="inputRef" @focus="generateNoise('input', $event)"
      @blur="removeNoise('input', $event)" />
  </div>
